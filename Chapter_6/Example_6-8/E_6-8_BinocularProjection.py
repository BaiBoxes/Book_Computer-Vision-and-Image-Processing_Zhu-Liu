...
------------------------
【例6-8】对所给图像进行标定后，利用cv2.projectPoints函数将三维世界坐标点投影到二维图像上。最后，展示并保存结果。
"""
...
# 定义示例的三维投影点
example_points_3D = objp

# 在矫正后的图像上投影点
projected_points_2D_left, _ = cv2.projectPoints(example_points_3D, rvecsL[0], tvecsL[0], cameraMatrix1, distCoeffs1)
projected_points_2D_right, _ = cv2.projectPoints(example_points_3D, rvecsR[0], tvecsR[0], cameraMatrix2, distCoeffs2)

for point in projected_points_2D_left:
    cv2.circle(imgL, tuple(point[0].astype(int)), 10, (0, 255, 0), -1)
for point in projected_points_2D_right:
    cv2.circle(imgR, tuple(point[0].astype(int)), 10, (0, 255, 0), -1)
