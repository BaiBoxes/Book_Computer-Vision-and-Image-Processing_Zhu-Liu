...
------------------------
【例6-9】首先对所给图像进行双目标定，再使用OpenCV库中的cv2.solvePnPRansac函数对校准结果进行双目姿态估计，最后，观察并保留其结果。
"""
import numpy as np
import cv2
import glob
...
if len(objpoints) > 0 and len(imgpoints_left) > 0 and len(imgpoints_right) > 0:
    ...
    # 使用solvePnPRansac计算左相机姿态
    retvalL, rvecL, tvecL, inliersL = cv2.solvePnPRansac(objp, imgpoints_left[0], cameraMatrix1, distCoeffs1)

    # 使用solvePnPRansac计算右相机姿态
    retvalR, rvecR, tvecR, inliersR = cv2.solvePnPRansac(objp, imgpoints_right[0], cameraMatrix2, distCoeffs2)

    # 将旋转向量转换为旋转矩阵
    R_matL, _ = cv2.Rodrigues(rvecL)
    R_matR, _ = cv2.Rodrigues(rvecR)
