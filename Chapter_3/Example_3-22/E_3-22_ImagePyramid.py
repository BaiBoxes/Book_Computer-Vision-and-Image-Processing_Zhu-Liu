...
------------------------
【例3-22】将图像用cv2.pyrDown函数进行3次高斯金字塔下采样，构建成高斯金字塔，并将高斯金字塔显示出来。
------------------------
"""
...
# 创建一个空列表，用于存储高斯金字塔的各个层
gaussian_pyramid = []
# 初始图像层加入到高斯金字塔
gaussian_pyramid.append(input_image)
# 依次进行3次高斯金字塔下采样
for i in range(3):
    # 使用cv2.pyrDown函数进行下采样
...
for i in range(4):
    axes[i].imshow(cv2.cvtColor(gaussian_pyramid[i], cv2.COLOR_BGR2RGB))
    axes[i].set_title(f'Layer {i}')
    axes[i].set_xticks([])
    axes[i].set_yticks([])
    axes[i].set_xlim(0, original_width)
    axes[i].set_ylim(original_height, 0)
plt.tight_layout()
plt.show()