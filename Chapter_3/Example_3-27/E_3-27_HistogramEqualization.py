...
------------------------
【例3-27】将图像用cv2.equalizeHis函数进行直方图均衡化，并将原图、处理后图与对应直方图显示出来。。
------------------------
"""
...
# 读取图像
image = cv2.imread('image/Example-Bridge_LowContrast.jpg', cv2.IMREAD_GRAYSCALE)
# 进行直方图均衡化
equalized_image = cv2.equalizeHist(image)
# 计算原始图像的直方图
hist_original = cv2.calcHist([image], [0], None, [256], [0, 255])
# 计算均衡化后图像的直方图
hist_equalized = cv2.calcHist([equalized_image], [0], None, [256], [0, 255])
# 显示原始图像的直方图
plt.figure(num='直方图均衡化', figsize=(12, 6))
plt.subplot(221)
plt.bar(np.arange(256), hist_original.reshape(256), width=1.0)
plt.title('Original Histogram')
# 显示均衡化后图像的直方图
plt.subplot(222)
plt.bar(np.arange(256), hist_equalized.reshape(256), width=1.0)
