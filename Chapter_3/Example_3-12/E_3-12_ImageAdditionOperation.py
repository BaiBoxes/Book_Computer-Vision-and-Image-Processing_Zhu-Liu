...
------------------------
【例3-12】分别用加号运算符“+”与cv2.add函数对图像进行加法运算，将提供的灰度图与自身相加，并显示出结果。
------------------------
"""
...
# 读取灰度图像
gray_image = cv2.imread('image/Example-Bridge_gray.jpg', cv2.IMREAD_GRAYSCALE)
# 使用加法运算符进行加法运算
result1 = gray_image + gray_image
# 使用cv2.add函数进行加法运算
result2 = cv2.add(gray_image, gray_image)
# 显示结果
cv2.imshow('gray_image', gray_image)
cv2.imshow('Using + Operator', result1)
cv2.imshow('Using cv2.add', result2)
cv2.waitKey(0)
