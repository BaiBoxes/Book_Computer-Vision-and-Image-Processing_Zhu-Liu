...
------------------------
【例3-13】将两幅图进行比较运算，通过cv2.max函数与cv2.min函数比较后取较大的像素值与取较小的像素值，分别组合成新的图像，并观察原图与合成的图的区别
------------------------
"""
...
# 读取两幅图像
image1 = cv2.imread('image/Example-Bridge.jpg')
image2 = cv2.imread('image/CV.jpg')
# 使用cv2.max函数获取两幅图像中较大的像素值
max_result = cv2.max(image1, image2)
# 使用cv2.min函数获取两幅图像中较小的像素值
min_result = cv2.min(image1, image2)
# 显示原图和比较后的图像
cv2.imshow('Image 1', image1)
cv2.imshow('Image 2', image2)
cv2.imshow('Max Result', max_result)
cv2.imshow('Min Result', min_result)
cv2.waitKey(0)
